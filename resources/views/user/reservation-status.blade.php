          ...
          <tbody class="text-center font-medium text-gray-900">
            @foreach ($reservations as $item)
              <x-table.process-body :id="$item->id" :no="$loop->iteration" :dateFiled="$item->created_at" :room="$item->room"
                ...
                :approvedAtTwo="$item->approved_by_bem_at" :isStepThreeApproved="$item->approved_by_kepala_jurusan" :approvedAtThree="$item->approved_by_kepala_jurusan_at" :isStepFourApproved="$item->status" :roomdownload="$item"
