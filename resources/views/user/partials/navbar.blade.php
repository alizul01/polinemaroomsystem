<header class="bg-gray-700 text-white flex flex-col gap-5 py-4 px-10">
    ...
    <hr>
    <nav class="mb-5">
        <div class="flex lg:flex-row flex-col gap-4 justify-between {{ request()->is('/') ? 'mb-24' : 'mb-[2.6rem]' }}">
                ...
                @if (Auth::user()->isAdmin())
                    <li>
                        <a href="{{ route('admin.pages.approval') }}"
                            class="no-underline {{ request()->is('approval*') ? 'text-white' : 'text-white/40 hover:text-white' }}">
                            <span>Approval</span>
                        </a>
                    </li>
                @endif
